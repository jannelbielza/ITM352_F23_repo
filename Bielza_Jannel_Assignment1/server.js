...
app.post("/process_form", function (request, response) {
   ...
   if (!haserrors) {
      if (hasquantities == true) {
         ...
         response.redirect("./invoice.html?" + queryStr.stringify(request.body));
      } else {
         //User will be kept on the page if the input is invalid
         response.redirect("./products_display.html?" + queryStr.stringify(request.body) + `&error=Please select some items`);
